...
export default function Layout({
  children, // 6. 이 레이아웃이 감싸고 있는 '실제 페이지 내용물'을 전달받습니다. (예: job/page.tsx의 내용)
}: Readonly<{
  ...
  // 즉, [헤더/푸터가 있는 틀] 안에 [채용 공고 페이지 내용]을 쏙 집어넣는 구조입니다.
  return <DefaultLayout>{children}</DefaultLayout>;
}

// 1. 요청 수신 및 메타데이터 처리 (Server Side Analysis)

// Next.js 서버가 /job 경로로 들어온 요청을 받습니다.

// 이 경로에 해당하는 layout.tsx 파일을 찾아서 읽습니다.

// 먼저 export const metadata를 확인합니다. 여기서 title: "다잇슈대전 | job" 정보를 가져와서, 최종적으로 생성될 HTML 문서의 <head><title>...</title></head> 부분에 끼워 넣습니다. (이 덕분에 검색엔진이나 브라우저 탭에 제목이 올바르게 뜹니다.)

// 2. 레이아웃 구조 형성 (Composition)

// 서버는 Layout 함수를 실행합니다.

// 이때, /job 경로의 실제 내용물인 page.tsx (채용 공고 리스트 등)를 실행하여 그 결과물을 children이라는 변수에 담습니다.
