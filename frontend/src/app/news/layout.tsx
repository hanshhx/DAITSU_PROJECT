...
// 뉴스 페이지에서도 동일한 헤더와 푸터를 사용하여 사용자가 사이트 내 어디에 있든 일관된 경험을 할 수 있도록 합니다.
...
export const metadata: Metadata = {
  ...
  // 사용자가 이 레이아웃이 적용된 페이지(뉴스 관련 페이지)에 접속하면, 탭 제목이 "다잇슈대전 | news"로 바뀝니다.
  title: "다잇슈대전 | news",
};
...
export default function Layout({
  children, // 6. 이 레이아웃이 감싸고 있는 '실제 페이지 내용물'을 전달받습니다. (예: news/page.tsx의 내용)
}: Readonly<{
  ...
  // 즉, [헤더/푸터가 있는 틀] 안에 [뉴스 목록 페이지 내용]을 쏙 집어넣는 구조입니다.
  return <DefaultLayout>{children}</DefaultLayout>;
}

// 1. 요청 수신 및 메타데이터 처리 (Server Side Analysis)

// Next.js 서버가 /news 경로로 들어온 요청을 받습니다.

// 이 경로에 해당하는 layout.tsx 파일을 찾아서 읽습니다.

// 먼저 export const metadata를 확인합니다. 여기서 title: "다잇슈대전 | news" 정보를 가져와서, 최종적으로 생성될 HTML 문서의 <head><title>...</title></head> 부분에 끼워 넣습니다.

// 2. 레이아웃 구조 형성 (Composition)

// 서버는 Layout 함수를 실행합니다.

// 이때, /news 경로의 실제 내용물인 page.tsx (뉴스 목록 리스트 등)를 실행하여 그 결과물을 children이라는 변수에 담습니다.
